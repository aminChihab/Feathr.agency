...
export function ResearchReportCard({ reportType, title, summary, createdAt, sections, defaultOpen = false }: ResearchReportCardProps) {
  ...
  const config = REPORT_TYPE_CONFIG[reportType] ?? { icon: Sparkles, label: reportType, color: 'bg-primary/10' }
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              <span className="bg-primary/15 text-primary text-[10px] uppercase tracking-wider rounded-full px-2 py-0.5 font-bold">Analysis</span>
              <span className="text-[10px] text-on-surface-variant/60">{relativeDate(createdAt)}</span>
            </div>
        ...
        <div className="border-t border-outline-variant/10 pt-5 mt-5 space-y-5">
          ...
          {summary && (
            <div className="rounded-lg bg-primary/5 p-4">
              <p className="text-sm text-on-surface-variant leading-relaxed whitespace-pre-wrap">{summary}</p>
            </div>
          )}
